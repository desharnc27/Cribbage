...
public class CombMeths {
    /**
     * After calling C(roof,vals.length) times,
     * iterates on all combinations of vals.length integers among those in {0...roof-1}
     * @param vals chosen integers of one combination
     * @param roof maximum value (excluded) a chosen integer can have
     * @return false if next iteration puts vals back to {0,1,2...}, true otherwise
     */
    ...
    /**
     * After calling tab.length! times,
     * iterates on all permutations of tab.length elements
    ...
    /**
     * After calling 2^tab.length times,
     * iterates on all 2^tab.length possible boolean array of length tab.length
     * @param tab
     * @return false if next iteration puts tab back to false at all indexes, true otherwise
     */
